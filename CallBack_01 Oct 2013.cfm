...
<cfif structKeyExists(url, "oauth_token") and structKeyExists(url, "oauth_verifier")> 
		...
		Your LinkedIn Account is verified and further process is in progress. <!---<a href="getConnections.cfm">Click here to get your connections</a>--->
		 ...
		 <cftry>
			 ...
			 <cfif IsArray(idArray) and ArrayIsEmpty(idArray)>
			       <cfoutput><br>
				        <span style="color: red;">
						   There is some issue in linkedin profile. Please try to sign up with linkedin again.
						</span> 
				   </cfoutput>
				   <cfabort>
			 <cfelse>
			 ...
			 <cfset pictureUrlArray = xmlSearch(results,'/person/picture-url')> 
			 <cfif IsArray(pictureUrlArray) and ArrayIsEmpty(pictureUrlArray) >
			      <cfset pictureUrl      = '0'>
			 <cfelse>   
				  <cfset pictureUrl      = #pictureUrlArray[1].XmlText#>
			 </cfif> 
			 ...
			 <cfset id = Replace(id, ",", "123")>
			 <cfset firstName = Replace(firstName, ",", "123")>
			 <cfset lastName = Replace(lastName, ",", "123")>
			 <cfset headline = Replace(headline, ",", "123")>
			 <cfset publicProfileUrl = Replace(publicProfileUrl, ",", "123")>
			 <cfset email = Replace(email, ",", "123")>
			 <cfset pictureUrl = Replace(pictureUrl, ",", "123")>
			 
			 <cfset data = #id# & ',' & #firstName# & ',' & #lastName# & ',' & #headline# & ',' & #publicProfileUrl# & ',' & #email# & ',' & #pictureUrl#>
			 ...
			 <cfif checkuseremail.recordcount> 
			            <cfquery name="updateDet" datasource="#dsn#" username="#request.username#" password="#request.password#">
						UPDATE Contacts
						  SET ContactStatus = 'Active',
						      FirstName = '#firstName#',
							  LastName  = '#lastName#',
							  linkedinprofilepic = '#pictureUrl#',
							  linkedinuser = 1
						WHERE ContactID = #checkuseremail.ContactID# 
				     </cfquery>
						...
						<cfset session.pictureUrl = TRIM(pictureUrl)>
					 ...
					 <script language="javascript">
					       alert("You already have an account and are being directed to your profile page.");
						   window.opener.location.href = "/index.cfm?fuseaction=profile&fusesubaction=docs&documentid=92";
						   window.close();
					 </script>
			 <cfelse> 
			         <script language="javascript">
						  window.opener.location.href = "/linkedInSignupStep2.cfm?cid=<cfoutput>#strEncodedString#</cfoutput>";
						  window.close();
					 </script>
			 </cfif> 
