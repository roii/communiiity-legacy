...
<td><a href="#request.webroot#/administration/dsp_IdeafunL_topic_add.cfm" target="editsection" onclick="fdbackwindow=window.open ('','editsection','toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=1,resizable=1,width=600,height=550,top=10,left=10')" style="color:###TRIM(AllSettings.BoxColour)#; text-decoration:none;"><strong>ADD A TOPIC &raquo;</strong></a>
	...
	<cfquery name="getCampaignName" datasource="#dsn#" username="#request.username#" password="#request.password#">
	SELECT * FROM campaigns WHERE CampaignsID = #trim(qry_all_categories.related_campaign)#
	</cfquery>
	...
	<td style="font-size:11px;" valign="top" align="center">#TRIM(SponsorStuff.Organisation_Name)# 
	</td> 
	...
	<tr>
	<td></td>
	<td style="font-size:11px;border-top:solid 1px ##e2e2e2;" valign="top"><br>#TRIM(qry_all_categories.Description)#<br><br /> 
		<b>Related Campaign</b>: #trim(getCampaignName.Title)#</td>
