       ...
       <script language="javascript">
	    ...
	    <form name="filterform" action="#request.webroot#/index.cfm?fuseaction=projects&fusesubaction=docs&documentID=#documentID#&groupID=#groupID#" method="post" onsubmit="return checkform();">
			...
			<!---<SELECT NAME="selCategory" onchange="location = this.options[this.selectedIndex].value;">
				<OPTION VALUE=""><cfoutput>#category#</cfoutput></OPTION>
			...
			<cfoutput> 
			    <input type="checkbox" name="statuselected" id="stat_1" value="#groupID#" /> &nbsp;#showall# <br />
				<div style="margin-left: 20px;">
					<input type="checkbox" name="statuselected" id="stat_2" value="Being Planned" /> &nbsp;
					 #beingplan#<br />
					<input type="checkbox" name="statuselected" id="stat_3" value="Available Now" /> &nbsp;
					 #availnow#<br />
					<input type="checkbox" name="statuselected" id="stat_4" value="In progress" /> &nbsp;
					 #inprog#<br />
					<input type="checkbox" name="statuselected" id="stat_5" value="All Done" /> &nbsp;
					 #alldone#
				</div>	 
		    </cfoutput>
			...
			<cfoutput> 
			    <input type="checkbox" name="priorityselected" id="prior_4" value="#groupID#" /> &nbsp;#showall#<br />
				<div style="margin-left: 20px;">
			...
			<input type="submit" value="#search# &gt;&gt;" class="whitefield" name="go">
		 </form>	
        </cfoutput>
  </div>
  <div class="rightwidth">
     <cfif ProjectDetails.recordcount gt 0>
			 ...
			 <cfif isDefined("FORM.selCategory")> 
					<cfif FORM.selCategory neq ''>
						   <cfif FORM.selCategory neq 'showallcats'>  
							  <cfquery name="getsearchCats" datasource="#dsn#" username="#request.username#" password="#request.password#">
										SELECT TITLE FROM ProjectCategories 
										WHERE ProjectCategoryID in (#FORM.selCategory#)
							  </cfquery> 
							  <div style="color: blue;">
									   You are searching for the Categories like as below:<br />
									   <cfoutput query="getsearchCats">
										   <span style="color: green; font-weight: bold;">
											  #getsearchCats.TITLE# &nbsp;<br />
										   </span>	
									   </cfoutput>
							  </div> 
						   <cfelse>
						      <div style="color: blue;">
									   You are searching for all the Categories 
							  </div>  	    
						   </cfif>	  
					</cfif> 
			 </cfif>
			    ...
			    <tr>
				     <td>
					      <span style="color:red;">
						        There is no records as per your search criteria. Please try again.
						  </span>
					 </td>
				</tr>
			</table>   		
...
SELECT P.ProjectID, P.INTROTEXT, P.ProjectCategoryID,P.Title,P.CurrentStatus,P.PaymentType,P.PaidIn,p.Priority,PC.Title ProjectCategory, P.CreditsOffered, P.projectpic, P.details, P.addedby, P.location, P.project_order, P.displayProject, P.PercentComplete
FROM Projects P INNER JOIN ProjectCategories PC on P.ProjectCategoryID = PC.ProjectCategoryID
WHERE P.ProjectCategoryID in (#ProjectCatIDList#) and 
      P.displayProject = 1
<cfif isdefined("url.selStatus")>
	<cfif url.selStatus neq ''>
		and currentstatus='#trim(url.selStatus)#'
