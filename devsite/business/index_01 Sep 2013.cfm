...
<cfif attributes.fusesubaction EQ 'index'>
	...
	<cfelse>
		<cflocation url="index.cfm?fuseaction=business&fusesubaction=docs&documentid=48">
	</cfif>

<cfelseif attributes.fusesubaction EQ 'BusinessDetails'> 

	<cfif session.UserIsAuthenticated EQ 'No'>
			
		<script>
		alert("You must be a registered member and logged-in to access these business templates.");
		history.back();
		</script>
	...
	<cfinclude template="dsp_bizidea_details.cfm">
			
<cfelseif attributes.fusesubaction EQ 'docs'>
	
	<cfparam name="BizMode" default="list">
	...
	<cfelseif attributes.documentID EQ '47'>
	
		<cfinclude template="../templates/dsp_document_details.cfm">		
		
		<cfif BizMode EQ 'list'>
			<cfinclude template="dsp_current_biz.cfm">
		<cfelseif BizMode EQ 'edit'>
			<cfinclude template="dsp_biz_transactions.cfm">
		<cfelseif BizMode EQ 'transaction'>
			<cfinclude template="dsp_add_transaction.cfm">
		<cfelseif BizMode EQ 'add'>
			<cfinclude template="dsp_add_biz.cfm">
		<cfelseif BizMode EQ 'EditTransaction'>
			<cfinclude template="dsp_edit_transaction.cfm">
		<cfelseif BizMode EQ 'EditBiz'>
			<cfinclude template="dsp_edit_biz.cfm">
		</cfif>		
			
	<cfelseif attributes.documentID EQ '48'>	
		<cfinclude template="../templates/dsp_document_details.cfm">		
		<cfif BizMode EQ 'list'>
			<cfinclude template="dsp_bizideas.cfm">		
		<cfelseif BizMode EQ 'details'>			
			<cfif session.UserIsAuthenticated EQ 'No'>			
				<script>
				alert("You must be a registered member and logged-in to access these business templates.");
				history.back();
		...
		<cfelseif BizMode EQ 'SectionDetails'>			
			<cfif session.UserIsAuthenticated EQ 'No'>			
				<script>
				alert("You must be a registered member and logged-in to access these business templates.");
				history.back();
			...
			<cfinclude template="dsp_bizidea_sectiondetails.cfm">
		</cfif>		
