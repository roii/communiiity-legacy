...
<cfif #cgi.http_host# eq "crowdsourciiing.com">
     <cflocation url="http://www.crowdsourciiing.com/devsite/" addtoken="no">
</cfif> 
...
<cfif IsDefined("URL.fuseaction") and URL.fuseaction neq ''>
     <cfset fuseaction = URL.fuseaction>
<cfelse>
     <cfset fuseaction = "homepage">
</cfif> 

<cfquery name="getPageTitle" datasource="#dsn#" username="#request.username#" password="#request.password#" maxrows="1">
	SELECT PageTitle, EnglishTitle, MenuEnglishTitle FROM Sections WHERE Fuseaction = '#fuseaction#'
</cfquery>

<cfset app.pageTitle = getPageTitle.ENGLISHTITLE>