...
<cfelse>
  <div class="container">
	...
	<td width="100%" style="padding-left:10px;padding-right:10px;" colspan="3">
		...
		<cfelseif attributes.fusesubaction EQ 'rate'>
		
			<cfif (session.UserIsAuthenticated EQ 'Yes' AND ListFind(TRIM(SectionDetails.ContactTypeView), session.User_ContactTypeID, ",")) OR (SectionDetails.PublicView EQ 1)>
		
		
				<cfinclude template="../templates/dsp_section_details.cfm">
				<cfparam name="type" default="BusinessTemplate">
			
				<cfif type CONTAINS 'BusinessTemplate'>
					<cfinclude template="dsp_rate_business_template.cfm">
				</cfif>	
