		...
		<cfif ValidContact.RecordCount>
			...
			<cfif AllWinners.RecordCount GT 1>
				...
				<cfloop query="AllWinners">
					...
					<!--- CHECK IF VALID CONTACT --->			
					<cfquery name="MultiValidContact" datasource="#dsn#" username="#request.username#" password="#request.password#">
					SELECT ContactID
					FROM Contacts
...
<cfset this_month_end = DaysInMonth(localdatetime)>
<cfset this_month_days = (this_month_end-1)>
...
<cfquery name="AddDraw" datasource="#dsn#" username="#request.username#" password="#request.password#">
INSERT INTO InviteFriendPrizes(DrawID, type, start_date, draw_date, status, winners, pointsvalue, showWinner, PointsPaid)
VALUES(#newid#, 'Month', #CreateODBCDate(localdatetime)#, #CreateODBCDate(DateAdd('d', this_month_days, localdatetime))#, 0, '', 10000, 0, 0)
 </cfquery>
	 
</cfoutput>

<cfquery name="AllReferralPrizeDraws" datasource="#dsn#" username="#request.username#" password="#request.password#" maxrows="1">
SELECT * 
FROM InviteFriendPrizes
WHERE Type LIKE '%Month%'
AND Status=1
...
<cfmail server="#application.emailserver#" to="alan@my3p.com" cc="roy@my3p.com" from="website@my3p.com" subject="Invite Friend- Month Draw successful" type="html">
RESULTS<br><br>
