...
<tr>
	 ...
	 <td width="75%" valign="top"> 
	        <input type="submit" name="submitSkills" value="Add" />
	 </td>
</tr> 
</table> 
</div>
...
function removeID(skillID, userid)
{ 
        $.ajax({
			type: "POST",
			url: "/updateprofile/remove_Skills.cfm",
			data: { skillID: skillID, userid: userid }
		})
		.done(function( msg ) { 
		    alert('Your Skill is deleted');
			if(msg == "Done")
			{  
			     $.ajax({
					type: "POST",
					url: "/updateprofile/reflect_select_skills.cfm",
					data: { userID: userID },
					beforeSend: function( xhr ) {
						 //xhr.overrideMimeType( "text/plain; charset=x-user-defined" );  
						 $('#selectedSkillsArray').html('<img src="/img/preview.gif" border="0" />');
					}
				})
				.done(function( msg ) {
					 $('#selectTextID').val('');
					 $('#selectedSkillsArray').html(msg);   
				});
			}
		});
}
...
$('form').submit(function(){ 
	...
	var selectTextID = $('#selectTextID').val();
	var userID = <cfoutput>#SESSION.userid#</cfoutput>;
	if(selectTextID != '')
	{    			
			$.ajax({
				type: "POST",
				url: "/updateprofile/check_Skills.cfm",
				data: { selectTextID: selectTextID }
			})
			.done(function( msg ) {
			    if(msg == 'Found')
				{  
				     alert('You didn\'t select the value from drop down as entered skill is already existed on the site.');
					 return false;
				}
				else
				{
				    // alert(msg); 
					     $.ajax({
							type: "POST",
							url: "/updateprofile/add_new_Skills_with_related_to_user.cfm",
							data: { selectTextID: selectTextID, userID: userID }
						})
						.done(function( msg ) {
								 $.ajax({
									type: "POST",
								...
								.done(function( msg ) {
									 $('#selectTextID').val('');
									 $('#selectedSkillsArray').html(msg); 
									 $('#mainulID').html('<li><input  type="text" value="" id="selectTextID" name="selectText" /></li>'); 
									 // top.window.location = '/index.cfm?fuseaction=updateprofile&fusesubaction=index&skilladd=1';  
								}); 
						}); 
				}
			}); 
			return false;
	      /*if(confirm("The skill is not available "))
		  {
	...
	{	
	       ...
	       /* 
			$('#skillsNeeded').val(data);  
			 ...
			 $.ajax({
				type: "POST",
				url: "/updateprofile/add_Skills.cfm",
				data: { data: data, userID: userID },
				beforeSend: function( xhr ) {
				     //xhr.overrideMimeType( "text/plain; charset=x-user-defined" );  
					 $('#mesgID').html('<img src="/img/preview.gif" border="0" />');
				}
			})
			.done(function( msg ) {
			    $('#mesgID').html('');
