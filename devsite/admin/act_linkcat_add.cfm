<!--- CREATE NEW ID --->
	...
	SELECT LinkCatID
	FROM LinkCats
	ORDER BY LinkCatID DESC
	</cfquery>
	
	
	<cfif #nextid.recordcount# GT 0>
		<cfset nextid = (#nextid.LinkCatID# + 1)>
	<cfelse>
		<cfset nextid = 1>
	</cfif>
...
<!--- INSERT CATEGORY  --->
	...
	INSERT INTO LinkCats(LinkCatID, Title, DisplayOrder, NonMember, Member, Released, Fuseaction)
	VALUES(#nextID#, '#form.Title#', #form.DisplayOrder#, <cfif #form.NonMember# EQ '0,1'>1<cfelse>0</cfif>, <cfif #form.member# EQ '0,1'>1<cfelse>0</cfif>, #form.released#, '#fuseaction#')
	</cfquery>
...
<!--- LOG THE ACTION --->
	...
	<cfoutput>
	<cfset logdetails = 'Add Link Category - (Category: #form.Title#)'>
