...
<table width="100%" cellpadding="0" cellspacing="0" style="border:solid 2px ###AllSettings.BoxColour#; padding: 10px;"> 
	<tr>
	          <td bgcolor="###session.AllSettings_BoxColour#" style="color:###TRIM(session.AllSettings_BoxTextColour)#;font-size:12px;" align="center" height="20"><strong>#managefootsite#</strong></td>
	</tr>
</cfoutput>		
	<tr>
	    <td> 
			   ...
			   SELECT * FROM site_section_contents_data WHERE section_id = 1
				...
				<form action="/administration/save_content.cfm?fuseaction=<cfoutput>#URL.fuseaction#</cfoutput>&fusesubaction=<cfoutput>#URL.fusesubaction#</cfoutput>" method="post">
				    ...
				    <input type="hidden" name="footer" value="1" />
						...
						<p> 
						  <cfoutput>
							<input type="submit" value="#save# »" class="whitefield" style="cursor:hand;background-color:###session.AllSettings_BoxColour#;color:###TRIM(session.AllSettings_BoxTextColour)#;font-weight:bold;font-size:11px;border:0px; height: 30px; width: 106px;">
						  </cfoutput>	
						</p>
					</form>
		</td>
