...
<cfif AllPrize.recordcount>
		...
		<cfif AllPrize.start_date neq ''>
		   <cfset this_start = AllPrize.start_date>
		<cfelse>
		   <cfset this_start = now()>    
		</cfif>
		
		<cfif AllPrize.draw_date neq ''>
...
<tr>
      <td colspan="7" height="2" style="color: red;">There is current not any Active prizes available</td>
</tr>
</cfif>
...
<cfif ArchivedPrizes.start_date neq ''>
   <cfset this_start = ArchivedPrizes.start_date>
<cfelse>
   <cfset this_start = now()>    
</cfif>

<cfif ArchivedPrizes.draw_date neq ''>
