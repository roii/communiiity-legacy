...
<td>
		...
		<table cellpadding="0" cellspacing="0" border="0" width="90%">
		   <tr>
				...
				<td>
						...
						<a href="#request.webroot#/projects/#URL.campaignID###tabs1-projects" style="text-decoration: none;">
						  <img width="32" border="0" height="32" title="back to tasks list" alt="back to tasks list" src="#request.webroot#/images/back-arrow.png">
						</a>
	...
	<tr>
	<td width="60" style="/*padding-right:10px;*/color:###TRIM(AllSettings.DkTextColor)#; text-align: left;" align="right"><strong>Skills Needed:</strong></td>
	<td>
	...
	<div style="background: none repeat scroll 0 0 ##7F7F7F; color: ##FFFFFF; float: left; padding: 4px 13px 4px 5px; width: 100%;">
	     <div style="width: 100%; float: left;">
		    ...
		    <span style="display: block; padding: 5px;">
				...
				<span style="padding-right: 10px; padding-left: 8px;">
				   <b>start date:</b>
				</span>
				<span style="border-right: 1px solid white;">
				   &nbsp;
						...
						<table width="100%" border="0" cellspacing="2" cellpadding="2" style="">
						     <tr style="background: none repeat scroll 0 0 ##FFFFFF; color: black;">
								...
								<td style="width: 10%; text-align: center;">
								   <cfif TRIM(ProjectDetails.CreateDate) neq ''> 
										#DateFormat(TRIM(ProjectDetails.CreateDate), "long")#
								   </cfif> 
								</td> 
								...
								<td style="width: 9%; text-align: center;">
								     <cfif TRIM(ProjectDetails.startdate) neq ''> 
										#DateFormat(TRIM(ProjectDetails.startdate), "long")#
								     </cfif> 
								</td> 
								...
								<td style="width: 9%; text-align: center;">
								    <cfif TRIM(ProjectDetails.finishdate) neq '' and ProjectDetails.finishdate neq "1900-01-01 00:00:00.000"> 
										#DateFormat(TRIM(ProjectDetails.finishdate), "long")#
								    </cfif> 
								</td> 
...
<script language="javascript">
   function deleteComments(commentID, ProjectCategoryID, ProjectID, campaignID)
   {
        var r = confirm("Are you sure you want to delete this comment!");
		if (r == true)
		  {
		        window.location = '#request.webroot#/tasks/delete_comments.cfm?commentID='+commentID+'&ProjectCategoryID='+ProjectCategoryID+'&ProjectID='+ProjectID+'&campaignID='+campaignID;
		  }
		else
		  {
		       return false;
   ...
   function editComments(commentData, CommentID)
   {  
		document.getElementById('Comment').innerHTML = commentData;
		document.getElementById('formType').value    = 'editC';
		document.getElementById('commentSubmit').value = 'Update Comments';
		document.getElementById('projectCommentID').value = CommentID;
   }
</script>

<cfif ProjectComments.RecordCount> 
<cfloop query="ProjectComments">
      <cfset authorID = ProjectComments.CONTACTID>
	  
	  <cfquery name="getAuthorProfilePic" datasource="#dsn#" username="#request.username#" password="#request.password#">
	      SELECT profilepic FROM Contacts WHERE CONTACTID = #authorID#
	  </cfquery>
	  
	    <cfset completed = ParseDateTime(now()) />
		<cfset started = ParseDateTime(trim(ProjectComments.DateAdded)) />
...
<tr>
	<td style="text-align:left;" valign="top"><!---#commentby#:---> 
	       <table cellpadding="0" cellspacing="0" border="0" width="100%">
		       <tr>
			        <td valign="top" width="10%">
					      <cfif IsDefined("getAuthorProfilePic.profilepic") and #getAuthorProfilePic.profilepic# neq ''>
								   <img src="#request.webroot#/images/profiles/#getAuthorProfilePic.profilepic#" width="75" height="75" border="0" />
						  <cfelse>
						           <img border="0" src="#request.webroot#/images/default_avatar.gif" width="75" height="75" style="color:##FFFFFF;" alt="No Image" title="No Image"> 
						  </cfif> 
					</td>
					<td valign="top" width="90%">	  
						  <div style="margin-bottom: 10px; margin-top: 4px;">
							  #ReReplace(trim(ProjectComments.Author),"\b(\w)","\u\1","ALL")# - 
							  <span style="color:##91AE4E;">
								  <cfif days gt 0>
									  <cfif days gt 6>
										   <strong>#DateFormat(trim(ProjectComments.DateAdded), "long")#</strong>   
									  <cfelse>
										   <strong>#days# days ago</strong>
									  </cfif>
								 <cfelse>
									  <cfif hours gt 0>
										   <strong>#hours# hours ago</strong>
									  <cfelse> 
											<cfif minutes gt 0>
												   <strong>#minutes# minutes ago</strong>
											<cfelse>
												   <strong>#seconds# seconds ago</strong>
											</cfif> 
									  </cfif>
								 </cfif>
							 </span>	 
						 </div>   
						 #TRIM(ProjectComments.Comment)# <br /><br />
							 <cfif IsDefined("SESSION.userid") and #SESSION.userid# eq #projectDetails.PROJECT_OWNER#>
							   <a href="javascript: void(0);" onclick="editComments('#TRIM(ProjectComments.Comment)#', #ProjectComments.CommentID#);">Edit Comments</a> &nbsp; <a href="javascript: void(0);" onclick="deleteComments(#ProjectComments.COMMENTID#, #url.ProjectCategoryID#, #TRIM(ProjectDetails.ProjectID)#, #URL.campaignID#);">Delete Comments</a>
							 </cfif>
					</td> 
			   </tr>
		   </table> 
...
<cfif IsDefined("SESSION.userid") and #SESSION.userid# gt 0>
	...
	<form action="#request.webroot#/index.cfm?fuseaction=tasks&fusesubaction=insertcomment" method="post" name="commentform">
	   <input name="formType" id="formType" type="hidden" value="addC" />
	   <input name="projectCommentID" id="projectCommentID" type="hidden" value="" />
	...
	<input name="Author" type="hidden" value="#SESSION.user_firstname# #SESSION.user_lastname#" />
	<input name="AuthorEmail" type="hidden" value="#SESSION.user_email#" />
	
	<!---#name#: <br /><input name="Author" type="text" /><br />
	#emailtext#: <br /><input name="AuthorEmail" type="text" /><br />
	#commenttitle#: <br />---><textarea name="Comment" id="Comment" cols="50" rows="5"></textarea><br />
	<!---Enter Text Below:<br />
	...
	<input type="submit" id="commentSubmit" value="#addcommenttitle#" onclick="validate(commentform); return false;" />
	
	</form>

<cfelse>
  
   <p style="font-weight: bold;">please login to comment</p>
