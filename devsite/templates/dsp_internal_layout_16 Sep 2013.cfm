...
<cfif attributes.fusesubaction EQ 'index'>
	...
	<cfif (session.UserIsAuthenticated EQ 'Yes' AND ListFind(TRIM(SectionDetails.ContactTypeView), session.User_ContactTypeID, ",")) OR (SectionDetails.PublicView EQ 1) OR (session.UserType EQ 'Administrator') OR fuseaction EQ 'Survey' OR fuseaction EQ 'updateprofile'>
