...
<tr>
<td width="100%" style="font-size:12px;color:###TRIM(AllSettings.DkTextColor)#" height="20">
       <strong>
	         <cfif IsDefined("Session.defaultlang") and Session.defaultlang neq ''>
				   <cfif Session.defaultlang eq 'fr'>
						 <cfset menutitle2Disp = #TRIM(SectionDetails.MenuFrenchTitle)#>
				   <cfelse>
						 <cfset menutitle2Disp = #TRIM(SectionDetails.MenuEnglishTitle)#>
				   </cfif>
			  <cfelse>
				   <cfset menutitle2Disp = #TRIM(SectionDetails.MenuFrenchTitle)#>
			  </cfif>
...
<!---<cfif (TRIM(sectiondetails.details) NEQ '') AND (sectiondetails.details NEQ '<P>&nbsp;</P>')>
	...
	<cfset str = Replace(#TRIM(sectiondetails.details)#, "</s", "")>
