<cfquery name="newjoined" datasource="#dsn#" username="#request.username#" password="#request.password#">
  select count(contactid) as newjoined from Contacts where datediff(hour,datejoined,getdate()) <= 24
</cfquery>
...
<cfquery name="newbiz" datasource="#dsn#" username="#request.username#" password="#request.password#">
  select count(bizid) as newbiz from bizkidsLink where datediff(hour,datecreated,getdate()) <= 24
</cfquery>
...
<cfquery name="newideas" datasource="#dsn#" username="#request.username#" password="#request.password#">
  select count(sugid) as newideas from Suggestion where datediff(hour,whensugmade,getdate()) <= 24
</cfquery>
...
<cfquery name="newgoals" datasource="#dsn#" username="#request.username#" password="#request.password#">
  select count(goalid) as total from goals where datediff(hour,addeddate,getdate()) <= 24
</cfquery>
...
<cfquery name="newtrans" datasource="#dsn#" username="#request.username#" password="#request.password#">
  select sum(income1_amount) as total from biztransaction where datediff(hour,tranDate,getdate()) <= 24
</cfquery>

<cfquery name="newprize" datasource="#dsn#" username="#request.username#" password="#request.password#">
  select sum(pointsvalue) as total from prize_pool where datediff(hour,start_date,getdate()) <= 24
</cfquery>
<cfquery name="newpoints" datasource="#dsn#" username="#request.username#" password="#request.password#">
	...
	and datediff(hour,transaction_date,getdate()) <= 24
</cfquery>
<cfquery name="newentry" datasource="#dsn#" username="#request.username#" password="#request.password#">
	  select prize_entryid from prize_entry where datediff(hour,entrydate,getdate()) <= 24
</cfquery>
<cfoutput>
<table width="100%" cellpadding="0" cellspacing="0" border="0" align="center">

<tr>
<td width="33%" align="center" style="border-right:solid 1px white;font-size:11px;color:black;"><strong>24hrs</strong></td>
<td width="33%" bgcolor="###AllSettings.BoxColour#" align="center" style="border-right:solid 1px white;font-size:11px;"><a href="##quickstats" onClick="javascript: SwapView('7', 0);" style="color:black;text-decoration:none;">7 days</a></td>
<td width="33%" bgcolor="###AllSettings.BoxColour#" align="center" style="color:white;font-size:11px;"><a href="##quickstats" onClick="javascript: SwapView('30', 0);" style="color:black;text-decoration:none;">30 days</a></td>
</tr>
...
<tr>
<td colspan="2"><hr wdith="100%" size="1" color="ffffff"></td>
</tr>
