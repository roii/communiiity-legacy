...
<cfset projectCategoryIDs = ListToArray(FORM.PROJECTCATEGORYID, ",")>
	...
	<cfquery name="getTasksInProject" datasource="#dsn#" username="#request.username#" password="#request.password#">
	       SELECT * FROM Projects WHERE related_campaign = #form.CampaignsID#
	</cfquery>
...
<!--- UPDATE --->
    ...
    <cfif getTasksInProject.recordcount gt 0>
		<cfoutput query="getTasksInProject">
			  <cfset projectID = #getTasksInProject.PROJECTID#>
			  <cfquery name="setTaskStatus" datasource="#dsn#" username="#request.username#" password="#request.password#">
					UPDATE Projects SET displayProject = #FORM.displaycampaigns# WHERE PROJECTID = #projectID#
			  </cfquery>
		</cfoutput> 
    </cfif>
