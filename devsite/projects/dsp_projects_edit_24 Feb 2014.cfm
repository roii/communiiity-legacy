...
<cfoutput query="qry_all_moderators_all">
	<script language="javascript">
		 tagsData[counter++] = "#TRIM(qry_all_moderators_all.Firstname)# #TRIM(qry_all_moderators_all.Lastname)# (#TRIM(qry_all_moderators_all.ContactID)# #TRIM(qry_all_moderators_all.ScreenName)#)";
	</script>
</cfoutput>
...
<td valign="top" colspan="3"> 
	...
	<td> </td>
	<td Class="small"><br><input type="submit" value="#save# >>" class="whitefield"> &nbsp; <input type="button" onClick="javascript:window.close();" value="#cancelbttn# >>" class="whitefield"></td>
	</tr>
	...
	<cfif session.UserIsAuthenticated EQ 'Yes' AND session.UserType EQ 'Administrator'>
		...
		<tr>
			...
			<td valign="top"><!---<select multiple name="Moderators" class="whitefield" style="width:300px;" size="6"> 
				...
				<cfif IsArray(userArr) and not ArrayIsEmpty(userArr)> 
					<cfloop query="qry_all_moderators_all">
							<cfif userArr[2] eq #qry_all_moderators_all.contactID#>
								<cfset userName = "#TRIM(qry_all_moderators_all.Firstname)# #TRIM(qry_all_moderators_all.Lastname)# (#TRIM(qry_all_moderators_all.ContactID)# #TRIM(qry_all_moderators_all.ScreenName)#)">
							</cfif>
					</cfloop>
				<cfelse>
				    <cfset userName = "">
				...
				<cfloop query="qry_all_moderators_all">
				        <script language="javascript">
						     tags[counter++]   = "#TRIM(qry_all_moderators_all.Firstname)# #TRIM(qry_all_moderators_all.Lastname)# (#TRIM(qry_all_moderators_all.ContactID)# #TRIM(qry_all_moderators_all.ScreenName)#)"; 
						</script>
				</cfloop>
				<script language="javascript">
				var testFlag = 0;  
