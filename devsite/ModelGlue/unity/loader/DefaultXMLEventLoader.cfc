...
<cfcomponent displayname="DefaultXMLEventLoader" output="f">

<cffunction name="init" returntype="ModelGlue.unity.loader.DefaultXMLEventLoader" output="true">
	<cfreturn this />
</cffunction>
...
<cffunction name="load" returntype="ModelGlue.unity.framework.ModelGlue" output="false" access="public">
	...
	<cflog file="mylog" text="parsed config" />
	...
	<cfif not arguments.include>
		<cfif arguments.mg.getConfigSetting("rescaffold") or not fileExists(arguments.scaffoldPath)>
	...
	<cfloop from="1" to="#arrayLen(arguments.eventHandlers.xmlChildren)#" index="i">
		...
		<cfswitch expression="#ehTag.xmlName#">
			...
			<cfcase value="scaffold">
				<cfif arguments.mg.getConfigSetting("rescaffold")>
