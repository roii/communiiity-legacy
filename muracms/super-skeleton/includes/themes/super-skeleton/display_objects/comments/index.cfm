...
<cfif not listFind("Portal,Gallery",$.content('type'))>
	...
	<div id="svComments">
		...
		<hr />
		<cfoutput>
		<h2 class="title"><span>#totalRecords# Responses</span></h2>
		...
		<form id="postcomment" method="post" name="addComment" action="?nocache=1#postcomment" onsubmit="return validate(this);" novalidate="novalidate">
			...
			<fieldset>
				<cfoutput><h2 id="postacomment" class="title"><span>#$.rbKey('comments.postacomment')#</span></h2>
				<h2 id="editcomment" style="display:none" class="title"><span>#$.rbKey('comments.editcomment')#</span></h2>
				<h2 id="replytocomment" style="display:none" class="title"><span>#$.rbKey('comments.replytocomment')#</span></h2>
				<ol>
					<li class="req">
					...
					<li id="rememberMe">
						<label for="cbRemember"><input type="checkbox" id="cbRemember" name="remember" value="1"<cfif isBoolean(cookie.remember) and cookie.remember> checked="checked"</cfif> />#$.rbKey('comments.rememberinfo')#</label>
						
					</li>
					<li id="subscribeComments">
						<label for="cbSubscribe"><input type="checkbox" id="cbSubscribe" name="subscribe" value="1"<cfif isBoolean(cookie.subscribe) and cookie.subscribe> checked="checked"</cfif> />#$.rbKey('comments.subscribe')#</label>
