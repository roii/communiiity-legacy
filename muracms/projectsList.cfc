...
<cffunction name="getProjectsList" access="remote" returntype="any" returnformat="plain" output="true">
	...
	<cfargument name="datasource" type="any" default="">
	...
	<cfquery name="records" datasource="#arguments.datasource#" username="mykidsbizcomi" password="u+67eNfJ">
	...
	<cfquery name="this_page" datasource="#arguments.datasource#" username="mykidsbizcomi" password="u+67eNfJ"> 
	   SELECT top #arguments.rows# ProjectCategories.Title as category, ProjectCategories.ProjectCategoryID as PCatID, *
		...
		<cfif isdefined("arguments.selStatus")>
			<cfif arguments.selStatus neq ''>
				and Projects.currentstatus='#trim(arguments.selStatus)#'
			</cfif> 
		</cfif>
		<cfif isDefined("arguments.selCategory")>
			<cfif arguments.selCategory neq ''>
				and Projects.ProjectCategoryID='#trim(arguments.selCategory)#'
			</cfif> 
		</cfif>
		<cfif isDefined("arguments.selPriority")>
			<cfif arguments.selPriority neq ''>
				and Projects.Priority='#trim(arguments.selPriority)#'
			</cfif> 
		</cfif>
	...
	<cfloop query="this_page"> <!---the grid wants the data inthis format [id,cell{column,column...}]--->
		...
		<cfset tmp2.cell[1] = this_page.ProjectID><!---?fuseaction=#arguments.fuseaction#&fusesubaction=#arguments.fusesubaction#&documentID=#arguments.documentID#&projectmode=template&ProjectCategoryID=#TRIM(this_page.PCatID)#&ProjectID=#TRIM(this_page.ProjectID)#--->
		<cfset tmp2.cell[2] = "<a href='#arguments.webroot#index.cfm/project-details/?documentID=#arguments.documentID#&ProjectCategoryID=#TRIM(this_page.PCatID)#&ProjectID=#TRIM(this_page.ProjectID)#'>"&this_page.INTROTEXT&"</a>">
