...
<cfif attributes.fusesubaction EQ 'campaignwidget'>
    <link rel="stylesheet" type="text/css" href="<cfoutput>#request.webroot#</cfoutput>/css/myCS.css"> 
    <cfinclude template="campaignwidget.cfm">
	<cfabort>
<cfelseif attributes.fusesubaction EQ 'campaigncreatewidget'>
    <link rel="stylesheet" type="text/css" href="<cfoutput>#request.webroot#</cfoutput>/css/myCS.css"> 
</cfif>
...
<cfif IsDefined("URL.campID") and URL.campID neq ''> 
    <cfset campaign_id = Replace(#TRIM(URL.campID)#, "-", " ", "ALL")>  
		...
		<title>
		   <cfif IsDefined("getCampaigns.TITLE") and #getCampaigns.TITLE# neq ''>
		        #TRIM(getCampaigns.TITLE)# | 
                               <cfif IsDefined("Session.defaultlang") and #Session.defaultlang# neq "">
                                         <cfif IsDefined("Session.defaultlang") and #Session.defaultlang# neq "fr">
				                 communautiii.com
				         <cfelse>
				                 communautiii.com 
				         </cfif> 
		   ...
		   <cfelse>
		        #TRIM(AllSettings.SiteName)# | Projects Page
		   </cfif>
		</title>
<cfelse> 
        <title><cfif IsDefined("this_metatitle") and this_metatitle neq ''>#TRIM(this_metatitle)#<cfelse>#TRIM(AllSettings.SiteName)# - Projects Page
		</cfif></title>
</cfif>
<!---<title><cfif IsDefined("this_metatitle") and this_metatitle neq ''>#TRIM(this_metatitle)#<cfelse>#TRIM(AllSettings.SiteName)# - Projects Page</cfif></title>--->
...
<script>(function(d, s, id) {
  ...
  js.src = "//connect.facebook.net/en_US/all.js#xfbml=1&appId=102035113170486";
...
<div style="width: 100%; background: none repeat scroll 0 0 ##f3f3f3;">

      <cfinclude template="../templates/header_include.cfm"> 
	  
	  <div class="content" style="background: none repeat scroll 0 0 ##f3f3f3;">
  ...
  <div class="container">
	...
	<tr>
		...
		<td width="100%" style="padding-left:10px;padding-right:10px;" colspan="3">
		 <cfif attributes.fusesubaction EQ 'addTask' or attributes.fusesubaction EQ 'addProject' or attributes.fusesubaction EQ 'editProject'>
		 <cfelse>	
				<cfif IsDefined("URL.campID") and #URL.campID# neq ''> 
				<cfelse>
					  <div style="font-size: 21px; font-weight: bold;">#browse# <span style="color: ##91AE4E;">#allproj#</span></div>  
				</cfif>
		 </cfif>	
		 <cfif attributes.fusesubaction EQ 'addProject'>
		       <a href="#request.webroot#/index.cfm?fuseaction=projects&documentID=5" style="text-decoration: none; color: black;">
			       Go Back to Project Listings
			   </a> 
			   <br><br>
		 <cfelseif attributes.fusesubaction EQ 'editProject'>	
		        <cfquery name="CampaignDetails" datasource="#dsn#" username="#request.username#" password="#request.password#">
					SELECT *
					FROM campaigns
					WHERE CampaignsID=#URL.CampaignsID#
				</cfquery>
		       <a href="#request.webroot#/projects/#URL.CampaignsID#" style="text-decoration: none; color: black;">
			   ...
			   <br><br>   
				...
				<cfelseif attributes.fusesubaction EQ 'addTask'>		
					  <cfinclude template="dsp_cashproject_add_from_projects.cfm">
				
				<cfelseif attributes.fusesubaction EQ 'addProject'>	
				      <cfinclude template="dsp_projects_add.cfm">
					  
				<cfelseif attributes.fusesubaction EQ 'editProject'>	
				      <cfinclude template="dsp_projects_edit.cfm">	  
				...
				<cfelseif attributes.fusesubaction EQ 'campaigncreatewidget'> 
					<cfinclude template="campaigncreatewidget.cfm">	
