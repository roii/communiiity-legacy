...
<cfif IsDefined("URL.search_keyword") and URL.search_keyword neq ''>
	 <cfset search_keyword = #URL.search_keyword#>
<cfelse>
...
<script language="javascript">
 ...
 function confirmProjectDelete(aURL) {
  ...
  function resetS()
  { 
       var linked = "<cfoutput>#webroot#</cfoutput>/projects/<cfoutput>#campaignID#</cfoutput>"; 
       top.location.href = linked;
  }
</script>
   ...
   <div class="leftwidth" style="width: 20%;">
        ...
        <cfoutput> 
	     ...
	     <form name="filterform" action="#webroot#/projects/display_tasks.cfm?ProjectCatIDList=#ProjectCatIDList#&webroot=#webroot#&fuseaction=#fuseaction#&fusesubaction=#fusesubaction#&documentID=#documentID#&allowTasks=#allowTasks#&campaignID=#campaignID#&filename=#filename#&userisauthenti=#userisauthenti#&dsn=#dsn#&username=#username#&password=#URL.password#&suserid=#suserid#&UserType=#UserType#" method="post" onsubmit="return checkform();">
			...
			<input type="submit" value="#search# &gt;&gt;" class="buttonControl" style="cursor: pointer;" name="go"><br />
	        <input type="button" value="Reset Search &gt;&gt;" class="buttonControl" style="cursor: pointer;" onclick="resetS();" name="go">
			<br />
			<br />
			<div> 
				  <span> 
				        <input type="text" maxlength="50" size="24" name="search_keyword" id="search_keyword" value="<cfif IsDefined("URL.search_keyword") and URL.search_keyword neq ''><cfoutput>#FORM.search_keyword#</cfoutput></cfif>">
				  </span>
			</div>
			<br />
			...
			<cfoutput> 
				...
				<div style="margin-left: 40px;" id="priorityBoxID">
				    <cfif IsDefined("URL.priorityselected") >
					     <cfset temp = ListFind(URL.priorityselected, 'High')> 
							<cfif temp is 0>  
						 ...
						 <cfset temp = ListFind(URL.priorityselected, 'Normal')> 
							<cfif temp is 0>  
						 ...
						 <cfset temp = ListFind(URL.priorityselected, 'Low')> 
							<cfif temp is 0>  
