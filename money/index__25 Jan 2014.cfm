...
<cfif attributes.fusesubaction EQ 'index'>
	...
	<cfif session.UserIsAuthenticated AND session.UserType NEQ 'Administrator'>
		
		<cflocation url="#request.webroot#/index.cfm?fuseaction=money&fusesubaction=docs&documentid=83">
	
	<cfelseif session.UserIsAuthenticated EQ 'No'>
		<cflocation url="#request.webroot#/index.cfm?fuseaction=money&fusesubaction=docs&documentid=84">
	</cfif>
...
<cfelseif attributes.fusesubaction EQ 'docs'>		
		...
		<cfif (session.UserIsAuthenticated) AND (DocumentID EQ 81 OR DocumentID EQ 82 OR DocumentID EQ 83)>
			<cfinclude template="../templates/dsp_document_details.cfm">
			<cfinclude template="dsp_kids_money.cfm">
		<cfelseif (session.UserIsAuthenticated) AND (documentID EQ 122)>
			
			<cfparam name="Purchasemode" default="Step1">
			
			<cfif PurchaseMode EQ 'Step1'>
				<cfset newdetails = DocumentDetails.details>
				<cfinclude template="dsp_points_purchase.cfm">
			
			<cfelseif PurchaseMode EQ 'Step2bank'>
				<cfinclude template="dsp_points_step2_bank.cfm">
			
			<cfelseif PurchaseMode EQ 'Step2payPal'>
				<cfinclude template="dsp_points_step2_paypal.cfm">
			
			<cfelseif PurchaseMode EQ 'confirmed'>
				<cfinclude template="dsp_points_confirmed.cfm">
			
			<cfelseif PurchaseMode EQ 'paymentconfirmed'>
				<cfinclude template="dsp_payment_confirmed.cfm">
			</cfif>
