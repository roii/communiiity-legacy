<cfquery name="newjoined" datasource="#dsn#" username="#request.username#" password="#request.password#">
  select count(contactid) as newjoined from Contacts where datediff(day,datejoined,getdate()) <= 7
</cfquery>
...
<cfquery name="newbiz" datasource="#dsn#" username="#request.username#" password="#request.password#">
  select count(bizid) as newbiz from bizkidsLink where datediff(day,datecreated,getdate()) <= 7
</cfquery>
...
<cfquery name="newideas" datasource="#dsn#" username="#request.username#" password="#request.password#">
  select count(sugid) as newideas from Suggestion where datediff(day,whensugmade,getdate()) <= 7
</cfquery>
...
<cfquery name="newgoals" datasource="#dsn#" username="#request.username#" password="#request.password#">
  select count(goalid) as total from goals where datediff(day,addeddate,getdate()) <= 7
</cfquery>
...
<cfquery name="newtrans" datasource="#dsn#" username="#request.username#" password="#request.password#">
  select sum(income1_amount) as total from biztransaction where datediff(day,tranDate,getdate()) <= 7
</cfquery>
<cfquery name="newprize" datasource="#dsn#" username="#request.username#" password="#request.password#">
  select sum(pointsvalue) as total from prize_pool where datediff(day,start_date,getdate()) <= 7
</cfquery>
<cfquery name="newpoints" datasource="#dsn#" username="#request.username#" password="#request.password#">
	...
	and datediff(day,transaction_date,getdate()) <= 7
</cfquery>
<cfquery name="newentry" datasource="#dsn#" username="#request.username#" password="#request.password#">
	  select prize_entryid from prize_entry where datediff(day,entrydate,getdate()) <= 7
</cfquery>
<cfoutput>
...
<tr>
<td width="33%" bgcolor="###AllSettings.BoxColour#" align="center" style="border-right:solid 1px white;font-size:11px;"><a href="##quickstats" onClick="javascript: SwapView('24', 0);" style="color:black;text-decoration:none;font-size:11px;">24hrs</a></td>
<td width="33%" align="center" style="border-right:solid 1px white;font-size:11px;color:black;"><strong>7 days</strong></td>
