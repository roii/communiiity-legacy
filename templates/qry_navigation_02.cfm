...
</cfquery><cfif ArticleStuff.RecordCount><cfset HasChild = 1><cfelse> ---><cfset HasChild = 0><!--- </cfif> ---><cfif (session.UserIsAuthenticated EQ 'Yes' AND ListFind(TRIM(DocumentStuff.ContactTypeView), session.User_ContactTypeID, ",")) OR (DocumentStuff.PublicView EQ 1) OR (session.UserType EQ 'Administrator')><cfset MenuItemString = '#TRIM(MenuItemString)#' & '#Replace(DocumentStuff.Title, ',', '&##44', "ALL")#' & ',' & 'index.cfm?fuseaction=#TRIM(thisfuseaction)#&fusesubaction=docs&documentID=#ThisDoc#' & ',' & '1' & ',' & '0' & ',' & '#HasChild#'><cfelse><cfset MenuItemString = "#TRIM(MenuItemString)#" & "<span title='Members only area'>#Replace(DocumentStuff.Title, ',', '&##44', "ALL")#</span>" & "," & "javascript:alert('Members only area. Please register or login to view this page.')" & "," & "1" & "," & "0" & "," & "#HasChild#"></cfif><cfset SubMenuItemString = '#TRIM(SubMenuItemString)#' & '_' & '#documentStuff.currentrow#' & ';'><!--- <cfloop query="ArticleStuff"><cfset ThisArt = ArticleStuff.ArticleID><cfif ArticleStuff.currentrow NEQ ArticleStuff.recordcount><cfif (session.UserIsAuthenticated EQ 'Yes' AND ListFind(TRIM(ArticleStuff.ContactTypeView), session.User_ContactTypeID, ",")) OR (ArticleStuff.PublicView EQ 1) OR (session.UserType EQ 'Administrator')><cfset SubMenuItemString = '#TRIM(SubMenuItemString)#' & '#TRIM(Replace(ArticleStuff.ArticleTitle, ',', '&##44', "ALL"))#' & ',' & 'index.cfm?fuseaction=#TRIM(thisfuseaction)#&fusesubaction=article&documentID=#ThisDoc#&articleID=#ThisArt#' & ',' & '1' & ';'><cfelse><cfset SubMenuItemString = "#TRIM(SubMenuItemString)#" & "<span title='Members only area'>#TRIM(Replace(ArticleStuff.ArticleTitle, ',', '&##44', "ALL"))#</span>" & "," & "javascript:alert('Members only area. Please register or login to view this page.')" & "," & "1" & ";"></cfif><cfelse><cfif (session.UserIsAuthenticated EQ 'Yes' AND ListFind(TRIM(ArticleStuff.ContactTypeView), session.User_ContactTypeID, ",")) OR (ArticleStuff.PublicView EQ 1) OR (session.UserType EQ 'Administrator')><cfset SubMenuItemString = '#TRIM(SubMenuItemString)#' & '#TRIM(Replace(ArticleStuff.ArticleTitle, ',', '&##44', "ALL"))#' & ',' & 'index.cfm?fuseaction=#TRIM(thisfuseaction)#&fusesubaction=article&documentID=#ThisDoc#&articleID=#ThisArt#' & ',' & '1' & ';'><cfelse><cfset SubMenuItemString = "#TRIM(SubMenuItemString)#" & "<span title='Members only area'>#TRIM(Replace(ArticleStuff.ArticleTitle, ',', '&##44', "ALL"))#</span>" & "," & "javascript:alert('Members only area. Please register or login to view this page.')" & "," & "1" & ";"></cfif></cfif></cfloop> ---></cfoutput></cfprocessingdirective>