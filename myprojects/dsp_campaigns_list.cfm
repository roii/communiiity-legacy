...
<cfelse>
	...
	SELECT P.CampaignsID, P.INTROTEXT, P.ProjectCategoryID,P.Title,P.CurrentStatus,P.PaymentType,P.PaidIn,p.Priority,PC.Title ProjectCategory, P.CreditsOffered, P.campaignspic, P.details, P.addedby, P.location, P.campaigns_order, P.displaycampaigns, P.PercentComplete
	FROM campaigns P INNER JOIN ProjectCategories PC on P.ProjectCategoryID = PC.ProjectCategoryID
	WHERE P.ProjectCategoryID in (#ProjectCatIDList#) and Moderators LIKE '%#session.userid#%'
	<cfif isdefined("FORM.selStatus")>
		<cfif FORM.selStatus neq ''> 
		   <cfif FORM.selStatus neq 'showallstatus'>  
			   and P.currentstatus IN ('#Replace(trim(FORM.selStatus), ",", "','", "ALL")#')
			    ...
			    <div style="margin-left: 40px;" id="categoryBoxID">
					<cfloop query="amarCatDetails">
					      <cfquery name="getProjectCatData" datasource="#dsn#" username="#request.username#" password="#request.password#">
                                SELECT count(*) as total FROM campaigns WHERE ProjectCategoryID = #amarCatDetails.ProjectCategoryID# and Moderators LIKE '%#session.userid#%'
                          </cfquery>
						<cfoutput> 
