...
<hr width="100%" size="1" color="e2e2e2">
	...
	<tr>
	<td><a href="#request.webroot#/index.cfm?fuseaction=#fuseaction#&fusesubaction=#fusesubaction#&kids_sortby=lastname&sortorder=<cfif session.sortorder EQ 'ASC'>DESC<cfelse>ASC</cfif>"><strong>Last</strong></a> <a href="#request.webroot#/index.cfm?fuseaction=#fuseaction#&fusesubaction=#fusesubaction#&kids_sortby=firstname&sortorder=<cfif session.sortorder EQ 'ASC'>DESC<cfelse>ASC</cfif>"><strong>First</strong></a> Name</td>
	<td width="50"><a href="#request.webroot#/index.cfm?fuseaction=#fuseaction#&fusesubaction=#fusesubaction#&kids_sortby=dateofbirth&sortorder=<cfif session.sortorder EQ 'ASC'>DESC<cfelse>ASC</cfif>"><strong>Age</strong></a></td>
	<td><a href="#request.webroot#/index.cfm?fuseaction=#fuseaction#&fusesubaction=#fusesubaction#&kids_sortby=DateJoined&sortorder=<cfif session.sortorder EQ 'ASC'>DESC<cfelse>ASC</cfif>"><strong>Date Joined</strong></a></td>
	<td><a href="#request.webroot#/index.cfm?fuseaction=#fuseaction#&fusesubaction=#fusesubaction#&kids_sortby=RegionID&sortorder=<cfif session.sortorder EQ 'ASC'>DESC<cfelse>ASC</cfif>"><strong>Region</strong></a></td>
	<td><a href="#request.webroot#/index.cfm?fuseaction=#fuseaction#&fusesubaction=#fusesubaction#&kids_sortby=Countries.Name&sortorder=<cfif session.sortorder EQ 'ASC'>DESC<cfelse>ASC</cfif>"><strong>Country</strong></a></td>
	...
	<cfquery name="AllClients" datasource="#dsn#" username="#request.username#" password="#request.password#">
	SELECT ClientID
	FROM KidClientLinks
	WHERE KidID=#allkids.ContactID#
	GROUP BY ClientID
	ORDER BY ClientID
	</cfquery>
	...
	<tr>
	<td<cfif allkids.ContactStatus EQ 'deleted'> style="text-decoration: line-through;"</cfif>>#LastName#, #FirstName#<cfif allkids.ContactStatus EQ 'pending'><span style="color:###TRIM(AllSettings.BoxColour)#;"> (pending)</span></cfif></td>
	<td<cfif allkids.ContactStatus EQ 'deleted'> style="text-decoration: line-through;"</cfif>>#DateDiff('yyyy', dateofbirth, localDateTime)#yrs</td>
	<td<cfif allkids.ContactStatus EQ 'deleted'> style="text-decoration: line-through;"</cfif>>#DateFormat(DateJoined, "d-mmm-yyyy")#</td>
	<td<cfif allkids.ContactStatus EQ 'deleted'> style="text-decoration: line-through;"</cfif>>#TRIM(Replace(regionstuff.region, " Region", ""))#</td>
	<td<cfif allkids.ContactStatus EQ 'deleted'> style="text-decoration: line-through;"</cfif>>#TRIM(Name)#</td>
	<td align="center"<cfif allkids.ContactStatus EQ 'deleted'> style="text-decoration: line-through;"</cfif>><cfif AllClients.RecordCount><a href="#request.webroot#/administration/dsp_kid_clients.cfm?kidsID=#contactID#" target="KidsClients" ONCLICK="fdbackwindow=window.open ('','KidsClients','toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=1,resizable=1,width=500,height=550,top=10,left=10')">#AllClients.RecordCount#</a><cfelse>#AllClients.RecordCount#</cfif></td>		
	<td align="right"><a href="#request.webroot#/admin/dsp_kid_edit.cfm?contactID=#contactID#" target="editContact" onclick="fdbackwindow=window.open ('','editContact','toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=1,resizable=1,width=700,height=450,top=10,left=10')"><img src="#request.imageroot#/but_edit_icon.gif" width="10" height="10" alt="Edit Kid" border="0"></a>&nbsp;<a href="javascript:confirmContactDelete('#request.webroot#/admin/act_delete_contact.cfm?contactID=#contactID#&caller=kids')"><img src="#request.imageroot#/but_delete_icon.gif" width="10" height="10" alt="Delete Kid" border="0"></a>&nbsp;<a href="javascript:confirmCompleteDelete('#request.webroot#/admin/act_delete_contact.cfm?contactID=#contactID#&caller=kids&completedelete=true')" title="Completely delete this kid from DB">D</a></td>
	</tr>
