...
<cfcomponent displayname="LinkedIn.cfc" hint="Functions to interact with LinkedIn API" output="yes">	
	<cfscript>
		variables.callback = "http://127.0.0.1/testLinkedin/CallBack.cfm"; // SET THIS TO THE FULL URL OF YOUR CALLBACK  
		variables.apiKey = "z397zaeyocxo";
		variables.secretKey = "ooGWjqTw2Hhch2Pm";		
		//DO NOT CHANGE THESE
	...
	<cffunction name="getRequestToken" access="public" output="no" returntype="void">
		<cfset var paramStr = "" />
		<cfset var signStr = "" />
		<cfset var signature = "" />
	...
	<cffunction name="getAccessToken" access="public" output="no" returntype="any">
		...
		--->
		<cfquery name="setUserAuth" datasource="#application.settings.dsn#">
			UPDATE tbl_User
			SET linkedInToken = '#listGetAt(authTokenList[1],2,"=")#', linkedInTokenSecret = '#listGetAt(authTokenList[2],2,"=")#'
			WHERE id = 5
		</cfquery>
	...
	<cffunction name="getAccessTokenInDetails" access="public" output="no" returntype="any">
		<cfargument name="authToken" required="yes" type="string" />
		<cfargument name="authVerifier" required="yes" type="string" />		
	...
	<cffunction name="getProfiles" access="public" output="no" returntype="any">
		...
		<cfset result = doLinkedInCommand(arguments.authToken, arguments.authTokenSecret, "http://api.linkedin.com/v1/people/#arguments.identifier#", "get") />
