...
<cfquery name="getEmail" datasource="#dsn#" username="#request.username#" password="#request.password#">
SELECT EmailAddress
FROM Contacts
WHERE ContactID = #form.ContactID#
</cfquery>

<cfset emailID = #getEmail.EmailAddress#>
...
SELECT ContactID
FROM Contacts
WHERE EmailAddress LIKE '#TRIM(emailID)#'
</cfquery>
...
<cftry>
	...
	ScreenName='#TRIM(form.FirstName)# #TRIM(form.LastName)#',
...
<cfset session.User_ScreenName = TRIM(NewMemberStuff.FirstName) & ' ' & TRIM(NewMemberStuff.LastName)>
