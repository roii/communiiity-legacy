<!---<cfquery name="topprofitbiz" datasource="#dsn#" username="#request.username#" password="#request.password#" cachedWithin="#CreateTimeSpan(0,0,15,0)#">
    SELECT top 3 biz.bizname,
	(income1_amount+income2_amount+income3_amount+income4_amount+income5_amount)/(hours/60) as avg_hr
	FROM biztransaction bt,bizkidslink bz, Business biz
	WHERE bt.bizkidslinkid = bz.bizkidslinkid and clientid <> 0 and (hours/60) > 0
	and bz.bizid=biz.bizid
	group by biz.bizname,(income1_amount+income2_amount+income3_amount+income4_amount+income5_amount)/(hours/60)
