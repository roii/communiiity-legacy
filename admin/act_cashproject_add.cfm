...
<!--- INSERT ARTICLE INFO --->
	...
	<cfquery name="getProjectCategoryID" datasource="#dsn#" username="#request.username#" password="#request.password#">
	     SELECT ProjectCategoryID FROM campaigns WHERE CampaignsID = #FORM.related_campaign#	
	</cfquery>
 
	
	<cfquery name="AddProject" datasource="#dsn#" username="#request.username#" password="#request.password#">
	INSERT INTO Projects(ProjectID, ProjectCategoryID, Title, related_campaign, introText, Details, CreditsOffered, FeatureProject, currentstatus, percentcomplete, assignedto, paymentType, PaidIn, projectpic, Priority, location, project_order, displayProject, addedby)
	VALUES(#newid#, #getProjectCategoryID.ProjectCategoryID#, '#form.title#', #FORM.related_campaign# , '#form.title#', '#form.details#', '#form.CreditsOffered#', #form.featureProject#, '#form.currentstatus#', '#form.percentcomplete#', '#form.assignedto#', '#form.PaymentType#', '#form.PaidIn#', '#localImage1#', 'Medium'
