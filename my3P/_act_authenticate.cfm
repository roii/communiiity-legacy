...
<cfloop query="Allpermissions">
	...
	<cfquery name="RolePermissions" datasource="#dsn#">
	SELECT *
	FROM RolePermissions
	WHERE RoleID=3
	AND PermissionID=#Allpermissions.PermissionID#
	</cfquery>
	
	<cfif RolePermissions.RecordCount>
		<cfif RolePermissions.AllowView EQ 1>
			<cfparam name="session.Permission#Allpermissions.PermissionID#_view" default="1">
		<cfelse>
			<cfparam name="session.Permission#Allpermissions.PermissionID#_view" default="0">
		</cfif>
		<cfif RolePermissions.AllowAdd EQ 1>
			<cfparam name="session.Permission#Allpermissions.PermissionID#_add" default="1">
		<cfelse>
			<cfparam name="session.Permission#Allpermissions.PermissionID#_add" default="0">
		</cfif>
		<cfif RolePermissions.AllowEdit EQ 1>
			<cfparam name="session.Permission#Allpermissions.PermissionID#_edit" default="1">
		<cfelse>
			<cfparam name="session.Permission#Allpermissions.PermissionID#_edit" default="0">
		</cfif>
		<cfif RolePermissions.AllowDelete EQ 1>
			<cfparam name="session.Permission#Allpermissions.PermissionID#_delete" default="1">
		<cfelse>
...
<cfif IsDefined("form.LoginName") AND IsDefined("form.Password") AND TRIM(form.Password) NEQ ''>
	...
	<cfquery name="Passwords" datasource="#dsn#" username="mykidsbizcom" password="w29snfc66r">
	SELECT *
	FROM Contacts
	...
	<cfif Passwords.RecordCount>
		...
		<cfelse>
			...
			<!--- GET USER ROLES --->
			<cfquery name="UserRoles" datasource="#dsn#">
			SELECT RoleID
			FROM contactRoles
			...
			<cfif UserRoles.RecordCount>
			
				<cfloop list="#UserRoles.RoleID#" index="thisRole" delimiters=",">
					...
					<cfloop query="RolePermissions">
						<cfif RolePermissions.AllowView EQ 1>
							<cfset "session.Permission#RolePermissions.PermissionID#_view" = 1>
						</cfif>
						<cfif RolePermissions.AllowAdd EQ 1>
							<cfset "session.Permission#RolePermissions.PermissionID#_add" = 1>
						</cfif>
						<cfif RolePermissions.AllowEdit EQ 1>
							<cfset "session.Permission#RolePermissions.PermissionID#_edit" = 1>
						</cfif>
						<cfif RolePermissions.AllowDelete EQ 1>
							<cfset "session.Permission#RolePermissions.PermissionID#_delete" = 1>
						</cfif>
					</cfloop>		
			...
			<cfif ListContains(userlist, session.User_ContactTypeID, ",")>
				...
				<cfif IsDate("#Passwords.LastLogin#")>
					...
					<cfif DateDiff('h', passwords.LastLogin, localDateTime) GT 24>
						...
						<!--- UPDATE LOGIN DATE --->
						<cfquery name="UpdateLogin" datasource="#dsn#" username="mykidsbizcom" password="w29snfc66r">
						UPDATE Contacts
						...
						<cfoutput>
							...
							self.location="#request.webroot#/index.cfm?fuseaction=profile&fusesubaction=docs&documentid=92";
							</script>
			...
			<cfelseif session.User_ContactTypeID EQ 6>
				...
				<!--- GET SCHOOL NAME --->
				<cfquery name="SchoolStuff" datasource="#dsn#" username="mykidsbizcom" password="w29snfc66r">
				SELECT SchoolName
				FROM Schools
			...
			<cfelseif session.User_ContactTypeID EQ 7>
				...
				<!--- GET SCHOOL NAME --->
				<cfquery name="PartnerStuff" datasource="#dsn#" username="mykidsbizcom" password="w29snfc66r">
